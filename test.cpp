...
struct Cars {
    ...
    int price [10] = {180,210,355,187,190,283,304,299,190,156};
...
void Details (int theChoice) {
   ...
   //  cout<<"\t\t\tMaximum Speed : "<<car.maxs_peed[theChoice-1]<<endl;
    cout<<"\t\t\tPrice for 30KM: Rs."<<car.price[theChoice-1]<<endl;
}
...
void user_input (int theChoice) {
   ...
   switch(choice) {
      case 1:
         cout << "\n\t\t\t You have chosen to pay now, You will be charged Rs."<<cost;
         MyFile << cost;
         break;

      case 2:
         cout << "\n\t\t\t You have chosen to pay later, You will be charged 25\% of the amount right now Rs." << cost * 0.25 << " You will need to pay the remaining Rs." << cost * 0.75 << "at the time of return";
         MyFile << cost * 0.25;
         cost = cost * 0.25;
         break;
   }
   ...
   string myText;
   ifstream MyReadFile("payment.txt");
   getline(MyReadFile, myText);
   // stringstream NumToStr(myText);

   // NumToStr >> pending_payment;
   cout << "\n\n\t\t\tWe are waiting for the payment of " << cost << " to arrive....";
   ...
   cout << "\n\n\t\t\tWe have recieved a payment of " << cost << "\n\t\t\tYour car has been booked!";
...
float calc_cost(float distance,int days){   
   float cost = ((distance - 30)*extra_fuel);
   return(cost);
}
////////////day 1//////////
